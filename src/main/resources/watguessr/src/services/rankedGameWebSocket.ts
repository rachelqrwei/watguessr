...
export function connectToRankedGame(gameId: string) {
  ...
  stompClient.connect({}, (frame) => {
    ...
    if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
      ...
      setTimeout(() => {
        if (currentGameId) {
          connectToRankedGame(currentGameId);
        }
      }, 3000);
    }
  });
...
export async function reconnectToGame(): Promise<boolean> {
  ...
  try {
    ...
    stompClient.send("/app/ranked-game/reconnect", {}, JSON.stringify(reconnectData));
    ...
    return await new Promise((resolve) => {
      ...
      const subscription = stompClient?.subscribe(`/topic/ranked-game/${gameInfo.gameId}/reconnect`, (message) => {
