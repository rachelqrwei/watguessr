...
public interface UserRepository extends JpaRepository<User, UUID> {
    ...
    @Query("SELECT u, " +
           ...
           "(:keyword IS NULL OR :keyword = '' OR u.username LIKE %:keyword%) " +
           "ORDER BY " +
           "CASE WHEN :sortBy = 'gamesWonDesc' THEN " +
           ...
           "CASE WHEN :sortBy = 'gamesLostDesc' THEN " +
           "(SELECT COUNT(DISTINCT g.id) FROM Game g JOIN Round r ON r.game.id = g.id JOIN Guess guess ON guess.round.id = r.id " +
           "WHERE g.winner IS NOT NULL AND g.winner.id != u.id AND g.gameMode = 'Ranked' AND guess.user.id = u.id) END DESC, " +
           "CASE WHEN :sortBy = 'winRateDesc' THEN " +
           "CASE WHEN (SELECT COUNT(DISTINCT g.id) FROM Game g JOIN Round r ON r.game.id = g.id JOIN Guess guess ON guess.round.id = r.id " +
           "WHERE guess.user.id = u.id AND g.winner IS NOT NULL) > 0 THEN " +
           "(SELECT COUNT(DISTINCT g.id) FROM Game g WHERE g.winner IS NOT NULL AND g.winner.id = u.id AND g.gameMode = 'Ranked') * 1.0 / " +
           "(SELECT COUNT(DISTINCT g.id) FROM Game g JOIN Round r ON r.game.id = g.id JOIN Guess guess ON guess.round.id = r.id " +
           "WHERE guess.user.id = u.id AND g.winner IS NOT NULL) ELSE 0 END END DESC, " +
           "CASE WHEN :sortBy = 'winRateAsc' THEN " +
           "CASE WHEN (SELECT COUNT(DISTINCT g.id) FROM Game g JOIN Round r ON r.game.id = g.id JOIN Guess guess ON guess.round.id = r.id " +
           "WHERE guess.user.id = u.id AND g.winner IS NOT NULL) > 0 THEN " +
           "(SELECT COUNT(DISTINCT g.id) FROM Game g WHERE g.winner IS NOT NULL AND g.winner.id = u.id AND g.gameMode = 'Ranked') * 1.0 / " +
           "(SELECT COUNT(DISTINCT g.id) FROM Game g JOIN Round r ON r.game.id = g.id JOIN Guess guess ON guess.round.id = r.id " +
           "WHERE guess.user.id = u.id AND g.winner IS NOT NULL) ELSE 0 END END ASC, " +
           "CASE WHEN :sortBy = 'createdAtAsc' THEN u.createdAt END ASC, " +
           "CASE WHEN :sortBy = 'createdAtDesc' THEN u.createdAt END DESC, " +
           "CASE WHEN :sortBy = 'streakDesc' THEN u.streak END DESC, " +
           "CASE WHEN :sortBy IS NULL OR :sortBy = '' OR :sortBy = 'elo' THEN u.elo END DESC")
